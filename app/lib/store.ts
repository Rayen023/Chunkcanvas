...
import {
  ...
  DEFAULT_VLLM_EMBEDDING_ENDPOINT,
  DEFAULT_DOCLING_ENDPOINT,
  DEFAULT_CHROMA_ENDPOINT,
  DEFAULT_FAISS_ENDPOINT,
} from "./constants";
...
export interface AppActions {
  ...
  setConfigForFile: (
    filename: string,
    config: Partial<ExtPipelineConfig>,
  ) => void;
...
export const useAppStore = create<AppState & AppActions>()(
  persist(
    (set, get) => ({
      ...
      faissIndexesDir: "/app/data",
      faissNewDbName: "index",
      faissDbPath: "/app/data/index.faiss",
      ...
      setFiles: (files) => {
        ...
        set({
          files,
          pipeline: vals[0] || "",
          pipelinesByExt: newPipelinesByExt,
          configByExt: newConfigByExt,
        });
      },
      addFiles: (newFiles) => {
        const state = get();
        ...
        set({
          files: nextFiles,
          pipelinesByExt: nextPipelinesByExt,
          configByExt: nextConfigByExt,
          pipeline: vals[0] || state.pipeline || "",
        });
      },
      removeFile: (index) => {
        const s = get();
        ...
        const remainingExts = new Set(
          next.map((f) => f.name.split(".").pop()?.toLowerCase() ?? ""),
        );
        ...
        if (
          ...
          if (
            s.embeddingsData &&
            s.embeddingsData.length === s.editedChunks.length
          ) {
        ...
        const uniquePipelines = [
          ...new Set(nextParsedResults.map((r) => r.pipeline)),
        ];
        const nextParsedDocType =
          nextParsedResults.length === 0
            ...
            : uniquePipelines.length === 1
              ? uniquePipelines[0]
              : "Mixed pipelines";
        const allExcelRows = nextParsedResults
      ...
      setConfigForExt: (ext, config) => {
        set((s) => {
          ...
          if (config.vllmEndpoint)
            nextGlobal.vllmEndpoint = config.vllmEndpoint;
          if (config.ollamaEndpoint)
            nextGlobal.ollamaEndpoint = config.ollamaEndpoint;
          if (config.doclingEndpoint)
            nextGlobal.doclingEndpoint = config.doclingEndpoint;
      ...
      setOllamaEndpoint: (ep) =>
        set((s) => {
          ...
          return {
            ollamaEndpoint: ep,
            configByExt: nextConfigByExt,
            lastConfigByExt: nextLastConfigByExt,
          };
        }),
      ...
      setVllmAdditionalEndpoints: (eps) =>
        set({ vllmAdditionalEndpoints: eps }),
      setVllmModel: (model) => set({ vllmModel: model }),
      setVllmPrompt: (prompt) => set({ vllmPrompt: prompt }),

      setDoclingEndpoint: (ep) =>
        set((s) => {
          const nextConfigByExt = { ...s.configByExt };
          for (const ext in nextConfigByExt) {
            nextConfigByExt[ext] = {
      ...
      setEmbeddingProvider: (provider) =>
        set({ embeddingProvider: provider, lastEmbeddingProvider: provider }),
      ...
      setOpenrouterEmbeddingModel: (model) =>
        set({ openrouterEmbeddingModel: model }),
      ...
      setEmbeddingsData: (data) =>
        set({
          embeddingsData: data,
          embeddingsForChunksHash: data ? get().chunksHash : null,
        }),
      ...
      setEmbeddingProgress: (pct, msg) =>
        set({ embeddingProgress: pct, embeddingProgressMsg: msg ?? "" }),
      ...
      setPineconeNamespaces: (namespaces) =>
        set({ pineconeNamespaces: namespaces }),
      ...
      setChromaCollections: (collections) =>
        set({ chromaCollections: collections }),
      ...
      resetAll: () => {
        ...
        set({
          ...
          openrouterEmbeddingModel:
            s.openrouterEmbeddingModel || "qwen/qwen3-embedding-8b",
          embeddingDimensions:
            s.embeddingDimensions || DEFAULT_EMBEDDING_DIMENSIONS,
          ollamaEmbeddingModel: s.ollamaEmbeddingModel || "",
          ollamaEmbeddingEndpoint:
            s.ollamaEmbeddingEndpoint || DEFAULT_OLLAMA_ENDPOINT,
          vllmEmbeddingModel: s.vllmEmbeddingModel || "",
          vllmEmbeddingEndpoint:
            s.vllmEmbeddingEndpoint || DEFAULT_VLLM_EMBEDDING_ENDPOINT,
          embeddingsData: null,
          embeddingsForChunksHash: null,
          embeddingChunkCache: {},
          embeddingsMeta: null,
          ...
          pineconeNamespaces: [],
          pineconeFieldMapping: s.pineconeFieldMapping || {
            idPrefix: "",
            textField: "text",
            filenameField: "filename",
          },
          ...
          faissIndexesDir: s.faissIndexesDir || "/app/data",
          faissNewDbName: s.faissNewDbName || "index",
          faissDbPath: s.faissDbPath || "/app/data/index.faiss",
